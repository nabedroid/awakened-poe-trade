...
const TRADE_WHISPER = {
  ...
  'jp': /^こんにちは、(?<league>.+) リーグで (?<price>.+) で売っている、あなたの (?<item>.+) を購入したいです \(スタッシュタブ "(?<tab_name>.*)"; 位置: 左から (?<tab_left>\d+), 上から (?<tab_top>\d+)\)(?<message>.+)?$/}
...
const TRADE_BULK_WHISPER = {
  ...
  'jp': /^こんにちは、私は(?<league>.+)であなたの(?<item>.+)を私の(?<price>.+)で購入したいです。$/
}
